...
// No libc dependency: operates on uint32/uint64/uint8 from types.h.
...
static void sha256_transform(sha256_ctx *ctx, const uint8 data[64]) {
  ...
  for (i = 0; i < 16; i++)
    m[i] = ((uint32)data[i * 4] << 24) | ((uint32)data[i * 4 + 1] << 16) |
...
void sha256_update(sha256_ctx *ctx, const void *vdata, uint len) {
...
void sha256_final(sha256_ctx *ctx, uint8 out[32]) {
  ...
  for (int j = 0; j < 8; j++)
    ctx->data[63 - j] = (uint8)(ctx->bitlen >> (j * 8));
  sha256_transform(ctx, ctx->data);

  for (int j = 0; j < 4; j++) {
    for (i = 0; i < 8; i++)
      out[i * 4 + j] = (uint8)(ctx->state[i] >> (24 - j * 8));
  }
}
