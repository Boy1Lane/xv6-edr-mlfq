...
clockintr()
{
  ...
  // Không cần acquire p->lock vì myproc() chỉ trả về tiến trình đang chạy
  // trên CPU hiện tại. Các trường uint8 (is_sandboxed, need_propagation)
  // được ghi nguyên tử trên RISC-V, __sync_synchronize() đảm bảo ordering.
  struct proc *p = myproc();
