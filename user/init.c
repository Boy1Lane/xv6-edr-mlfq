...
main(void)
{
  ...
  for(;;){
    if (edr_pid < 0) {
      // Testing hook: if /edroff exists, do not (re)spawn the EDR daemon.
      // Checked on every respawn so automated tests can stop the daemon
      // reliably by creating the flag and killing it once.
      int fd = open("edroff", O_RDONLY);
      if(fd >= 0){
        close(fd);
      } else {
        edr_pid = fork();
