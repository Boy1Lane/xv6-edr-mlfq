...
int main(int argc, char *argv[]) {
  ...
  // "once": in một snapshot rồi thoát (dùng cho automated tests).
  int once = (argc > 1 && strcmp(argv[1], "once") == 0);

  // Tên trạng thái để dễ đọc hơn
  static const char *state_names[] = {"UNUSED", "USED", "SLEEP", "RUN", "RUNNING", "ZOMBIE"};
  ...
  while (1) {
    ...
    int dropped = info.alerts_dropped;
      ...
      if (info.state[i] != 0) {
        ...
        if (info.is_sandboxed[i] > 0) {
          ...
          printf("\x1b[31m%d\t%d\t%d\t%d\t%s\t[QUARANTINE]\x1b[0m\n", info.pid[i], info.priority[i],
                 info.ticks_used[i], info.total_runtime[i], state_str);
        } else {
          printf("%d\t%d\t%d\t%d\t%s\n", info.pid[i], info.priority[i], info.ticks_used[i],
                 info.total_runtime[i], state_str);
        }
      }
    }

    if (dropped > 0)
      printf("\x1b[33m[EDR] %d alerts dropped (buffer overflow)\x1b[0m\n", dropped);
